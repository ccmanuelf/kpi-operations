...
| 11 | Work Order Management | `frontend/src/views/WorkOrderManagement.vue` (+ `frontend/src/composables/useWorkOrderGridData.ts`) | `/work-orders` | Work orders (CRUD, QC approval, capacity link) | `POST /work-orders`, `PUT /work-orders/{id}`, `PATCH /work-orders/{id}/status`, `POST /work-orders/{id}/approve-qc`, `POST /work-orders/{id}/link-capacity`, `POST /work-orders/{id}/unlink-capacity` | AG Grid (inline) | **Compliant** (migrated 2026-05-01, Group H Surface #19) | Replaced the v-data-table list + 12-field create/edit dialog with `AGGridBase` + new `useWorkOrderGridData` composable (Group E inline-edit pattern applied to operational data). Existing rows autosave PUT on cell-value change; new rows accumulate locally and POST on the row's green Save button. `work_order_id` is editable only on new rows. `client_id` resolved from auth/kpi store (fixes legacy hardcoded `'CLIENT001'`). Status/priority use `agSelectCellEditor` against backend enum catalogues (`WORK_ORDER_STATUS_OPTIONS`, `WORK_ORDER_PRIORITY_OPTIONS`). Filters (search/status/priority/dates), summary stat cards, detail drawer (Exception 1), delete confirmation (Exception 4) and more-actions status menu (workflow API) preserved. Tests in `useWorkOrderGridData.spec.ts` (30 tests). |
| 12 | My Shift Dashboard | `frontend/src/views/MyShiftDashboard.vue` (uses `frontend/src/components/dialogs/ShiftDashboardDialogs.vue` + `frontend/src/composables/useShiftForms.ts`) | `/my-shift` | Quick production / downtime / quality entries during a shift | `POST /production`, `POST /downtime`, `POST /quality` | Form (quick-action dialogs) | **Compliant** (migrated 2026-05-01, option a) | Per Group C / Surface #8: previous submit handlers in `useShiftForms.ts` sent payloads that 422'd (wrong field names, missing required fields). Now reconciled to backend Pydantic schemas using the Group A pattern: `client_id` from auth/kpi store; product_id resolved via `style_model` → product_code lookup (fallback to first product); shift_id resolved from active shift (fallback to first shift); UI downtime reason mapped to `DOWNTIME_REASON_CODES` enum via `downtimeReasonToCode()` helper; `units_passed` computed for quality. Reference data (`/products`, `/shifts`) loaded once on composable mount. Quick-action UX preserved (operators still log a single entry from the FAB without leaving the dashboard). Help dialog unchanged (Exception 4 — single-action, no persistence). Tests in `useShiftForms.spec.ts` (24 tests). |
...
| 20 | Capacity — Orders (worksheet grid) | `frontend/src/views/CapacityPlanning/components/grids/OrdersGrid.vue` | tab inside `/capacity-planning` | Customer orders for capacity scheduling | `POST /capacity/orders`, `PUT /capacity/orders/{id}`, `PATCH /capacity/orders/{id}` (persistence centralised via `useCapacityPlanningStore.saveWorksheet('orders')`) | Grid (AG Grid Community) | **Compliant** (migrated 2026-05-01) | Rebuilt on `AGGridBase` with composable `useOrdersGridData.ts`. Native AG Grid editors per backend `OrderCreate` schema: `agTextCellEditor` for order_number (pinned left)/customer_name/style_model; `agNumberCellEditor` (min:0, precision:0) for order_quantity; `agDateStringCellEditor` for required_date; `agSelectCellEditor` over canonical `ORDER_PRIORITY_OPTIONS` (LOW/NORMAL/HIGH/URGENT — fixes legacy `CRITICAL` UI bug that didn't match backend OrderPriority enum); status as read-only chip with color renderer over canonical `ORDER_STATUS_OPTIONS` (DRAFT/CONFIRMED/SCHEDULED/IN_PROGRESS/COMPLETED/CANCELLED). CSV-paste import preserved via `parseCsv` helper (defaults non-canonical priorities to NORMAL). Tests in `useOrdersGridData.spec.ts` (25 tests). |
| 21 | Capacity — Standards (worksheet grid) | `frontend/src/views/CapacityPlanning/components/grids/StandardsGrid.vue` | tab inside `/capacity-planning` | Production standards (SAM, setup, machine time) | `POST /capacity/standards`, `PUT /capacity/standards/{id}` (persistence centralised via `useCapacityPlanningStore.saveWorksheet('productionStandards')`) | Grid (AG Grid Community) | **Compliant** (migrated 2026-05-01) | Rebuilt on `AGGridBase` with composable `useStandardsGridData.ts`. Native AG Grid editors per backend `StandardCreate` schema: `agTextCellEditor` for style_model/operation_code (both pinned left)/operation_name/department; `agNumberCellEditor` (min:0, precision:2) for sam_minutes, setup_time_minutes, machine_time_minutes; pinned-right actions column with duplicate + delete. Tests in `useStandardsGridData.spec.ts` (15 tests). |
| 22 | Capacity — Calendar grid | `frontend/src/views/CapacityPlanning/components/grids/CalendarGrid.vue` | tab inside `/capacity-planning` | Master production calendar (working day flag, shifts available, shift hours, holiday name) | `POST /capacity/calendar`, `PUT /capacity/calendar/{id}` (persistence centralised via `useCapacityPlanningStore.saveWorksheet('masterCalendar')`) | Grid (AG Grid Community) | **Compliant** (migrated 2026-05-01) | Rebuilt on `AGGridBase` with composable `useCalendarGridData.ts`. Native AG Grid editors per backend `CalendarEntryCreate` schema: `agDateStringCellEditor` for calendar_date (pinned left); `agCheckboxCellEditor` for is_working_day with green-check renderer; `agSelectCellEditor` over [1,2,3] for shifts_available; `agNumberCellEditor` (min:0, max:24, precision:2) for shift1_hours and shift2_hours; `agTextCellEditor` for holiday_name. Conditional editability: shift2_hours only editable when `shifts_available >= 2` (preserves legacy disabled binding). Generate-Month dialog UX preserved (year + month picker auto-populates entries with weekend/weekday defaults). Tests in `useCalendarGridData.spec.ts` (19 tests including leap-year February + weekend detection). |
| 23 | Capacity — BOM grid | `frontend/src/views/CapacityPlanning/components/grids/BOMGrid.vue` | tab inside `/capacity-planning` | Bill of Materials (master + components) | `POST /capacity/bom`, `PUT /capacity/bom/{id}`, `POST /capacity/bom/{id}/components`, `PUT /capacity/bom/{id}/components/{cid}` (persistence centralised via `useCapacityPlanningStore.saveWorksheet('bom')`) | Grid (AG Grid Community, master-detail) | **Compliant** (migrated 2026-05-01) | Rebuilt on stacked-grid master-detail with composable `useBOMGridData.ts`. AG Grid Community has no native master-detail (Enterprise-only) so two AGGridBase grids are stacked: a parent grid for BOM headers (parent_item_code pinned left, parent_item_description, style_model, revision, components-count read-only via valueGetter, is_active checkbox) and a detail grid for the selected BOM's components (component_item_code pinned left, description, quantity_per number with precision:4, unit_of_measure select over `BOM_UOM_OPTIONS`, waste_percentage 0-100 with precision:2). Click a parent row → detail grid populates. Picks up the previously-orphaned `POST /capacity/bom/{id}/components` and `PUT /capacity/bom/{id}/components/{cid}` endpoints (Phase 0 § 6.7 finding). Tests in `useBOMGridData.spec.ts` (26 tests covering master+detail column shapes, selection state, CRUD delegation, and selection shifting after row removal). |
...
| 28 | Capacity — KPI Tracking panel | `frontend/src/views/CapacityPlanning/components/panels/KPITrackingPanel.vue` | tab inside `/capacity-planning` | KPI commitment tracking (target vs actual + variance + status) | `PUT /capacity/kpi-workbook/...`, `PUT /kpi/thresholds` (persistence centralised via `useCapacityPlanningStore.saveWorksheet('kpiTracking')`) | Grid (AG Grid Community) | **Compliant** (migrated 2026-05-01) | Rebuilt on `AGGridBase` with composable `useKPITrackingGridData.ts`. Editable: kpi_name (text, pinned left), target_value (number). Read-only with chip renderers: actual_value, variance_percent (color-coded by `classifyVariance` helper — ON_TARGET ≤5%, OFF_TARGET ≤10%, CRITICAL >10%), status (color-coded across PENDING/ON_TRACK/AT_RISK/OFF_TARGET/ACHIEVED), period (date-range valueGetter). Read-only fields populated by `store.loadKPIActuals(period)` against backend actuals data. Load Actuals period-picker dialog preserved as Exception 3. Tests in `useKPITrackingGridData.spec.ts` (21 tests including variance classification at the bucket boundaries). |
...
| 30 | Capacity — Scenarios panel | `frontend/src/views/CapacityPlanning/components/panels/ScenariosPanel.vue` (+ `frontend/src/composables/useScenariosGridData.ts`) | tab inside `/capacity-planning` | Scenario creation (what-if) | `POST /capacity/scenarios`, `POST /capacity/scenarios/{id}/run`, `POST /capacity/scenarios/compare`, `DELETE /capacity/scenarios/{id}` | AG Grid (inline, new rows only) | **Compliant** (migrated 2026-05-01, Group H Surface #20) | Replaced the card grid + 16-input `v-dialog` with `AGGridBase` + new `useScenariosGridData` composable. Scenarios are point-in-time records (DRAFT → EVALUATED → APPLIED/REJECTED) and the backend has no update endpoint, so only **new rows** are editable: name (text), type (`agSelectCellEditor` against `SCENARIO_TYPE_OPTIONS` mirroring backend ScenarioCreate), parameters (`agLargeTextCellEditor` popup — JSON object). Type-switch resets parameters to that type's defaults via `valueSetter`, mirroring the legacy dialog's per-type input swap. Existing rows expose Run (DRAFT only) and Delete actions; multi-row selection enables the top-bar Compare button (`store.compareScenarios(...)`). Delete confirmation dialog kept (Exception 4). **Bug fix included:** `capacityPlanning.ts:createScenario` was sending `name` instead of `scenario_name` — silent 422 in legacy dialog because errors were swallowed. Tests in `useScenariosGridData.spec.ts` (33 tests). |
...
| 34 | Admin — Defect Types | `frontend/src/views/admin/AdminDefectTypes.vue` | `/admin/defect-types` | Per-client defect catalog | `POST /defect-type-catalog`, `PUT /defect-type-catalog/{id}`, `POST /defect-type-catalog/upload/{client_id}` | Grid (AG Grid Community) | **Compliant** (migrated 2026-05-01) | Rebuilt on `AGGridBase` with composable `useDefectTypesGridData.ts`. Inline-editing pattern: existing rows autosave PUT on every cell-value change (Excel-style); new rows accumulate locally until the operator clicks the green Save button in the row's actions column, then POST. Native AG Grid editors per backend defect-type schema: `agTextCellEditor` for defect_code (pinned left, editable only on new rows — matches legacy "disabled when editing" semantics) / defect_name / industry_standard_code; `agLargeTextCellEditor` for description; `agSelectCellEditor` over `CATEGORY_OPTIONS` (15 categories) for category; `agSelectCellEditor` over `SEVERITY_OPTIONS` (CRITICAL / MAJOR / MINOR) with color-coded chip renderer for severity_default; `agNumberCellEditor` (min:0) for sort_order; `agCheckboxCellEditor` for is_active. Required-field validation client-side before POST (defect_code + defect_name + severity_default). Delete confirmation + CSV upload + template download dialogs preserved (all qualify as Exception 4 / Exception 3 surfaces). Tests in `useDefectTypesGridData.spec.ts` (23 tests). |
| 35 | Admin — Part Opportunities | `frontend/src/views/admin/PartOpportunities.vue` | `/admin/part-opportunities` | Per-part opportunity classification | `POST /part-opportunities`, `PUT /part-opportunities/{id}`, `POST /part-opportunities/upload` | Grid (AG Grid Community) | **Compliant** (migrated 2026-05-01) | Rebuilt on `AGGridBase` with composable `usePartOpportunitiesGridData.ts`. Same Excel-style inline-edit pattern as Defect Types (Surface #14): existing rows autosave PUT on cell change; new rows accumulate locally with explicit Save. Native AG Grid editors per backend schema: `agTextCellEditor` for part_number (pinned left, editable only on new rows); `agNumberCellEditor` (min:1) for opportunities_per_unit; `agTextCellEditor` for part_description; `agSelectCellEditor` over `COMPLEXITY_OPTIONS` (Simple/Standard/Complex/Very Complex) for complexity; `agSelectCellEditor` over `clientOptions` for client_id (editable only on new rows; valueFormatter resolves client_name); `agLargeTextCellEditor` for notes; `agCheckboxCellEditor` for is_active. Required-field validation (part_number + client_id + opportunities_per_unit > 0) before POST. Delete confirmation, CSV upload, and how-to guide dialogs preserved. Tests in `usePartOpportunitiesGridData.spec.ts` (21 tests). |
...
| 37 | Admin — Floating Pool | `frontend/src/views/admin/FloatingPoolManagement.vue` (+ `frontend/src/composables/useFloatingPoolGridData.ts`) | `/admin/floating-pool` | Floating-pool employee assignment, simulation | `POST /floating-pool/assign`, `POST /floating-pool/unassign` | AG Grid (inline) | **Compliant** (migrated 2026-05-01, Group H Surface #21) | Replaced the v-data-table + 5-field assign/edit dialog with `AGGridBase` + new `useFloatingPoolGridData` composable. Each row is a pool entry; pool membership is owned by the employee admin surface so this view has no Add Row. Inline-edit policy: `current_assignment` (Client column, agSelectCellEditor against the loaded client list) — clearing fires `/floating-pool/unassign`, setting fires `/floating-pool/assign` with the row's current dates+notes. Date / notes edits on an assigned row re-fire `/assign` (matches the legacy "edit dialog re-POST" semantics); the same edits on an unassigned row are local-only (no endpoint to set window without a client). Quick-action `Unassign` button rendered when assigned. Summary cards, status/client filters, simulation insights expansion panel and the How-to-Use guide dialog (Exception 4 — read-only) are preserved. Dead `useFloatingPoolForms` composable removed. Tests in `useFloatingPoolGridData.spec.ts` (21 tests). |
...
| 46 | Workflow Wizard — Targets step | `frontend/src/components/workflow/steps/WorkflowStepTargets.vue` | shift wizard | Shift-start work-order review checkpoint | `GET /work-orders` (read-only) | Other (read-only checkpoint) | **Compliant** (migrated 2026-05-01, option c) | The migration plan §5.G envisaged a 24-hour target grid, but on inspection the live step was a Group B-style review surface targeting nonexistent `GET /work-orders/shift-queue` and `/materials/availability` endpoints with mock-data fallback. Treated with the same option (c) pattern as Group B: now fetches canonical `GET /work-orders?client_id=<auth>`, maps `WorkOrderResponse` fields (work_order_id/style_model/planned_quantity/actual_quantity/planned_ship_date) into the display shape, computes progress = actual / planned × 100, surfaces fetch errors via `useNotificationStore`. Materials section removed (no backend representation). Tests in `WorkflowStepTargets.spec.ts` (11 tests, includes regression test that no mock data is injected on fetch failure). |
...
| 49 | Workflow Wizard — Attendance step | `frontend/src/components/workflow/steps/WorkflowStepAttendance.vue` | shift wizard (shift-start) | Roster review (read-only) | `GET /employees?active=true&client_id=<auth>` (read-only) | Other (read-only checkpoint) | **Compliant** (migrated 2026-05-01, option c) | Per Group B Phase 0: previous step targeted nonexistent `GET /employees/shift-roster` and `GET /stations`; never persisted attendance. Now refactored to a read-only roster checkpoint that fetches the canonical `/api/employees?active=true&client_id=<auth>`, maps `employee_id`/`employee_name`/`role` to local shape, defaults all employees to present, lets the operator toggle present/absent, gates wizard advance on ≥80% present, and links out to `/data-entry/attendance` (Group A grid) for actual persistence. Station-assignment section removed (no backend support; UI-only state was being silently discarded). Wired `useNotificationStore` for fetch errors. Mock-fixture fallback removed (8 fake employees + 5 fake stations). Tests in `WorkflowStepAttendance.spec.ts` (10 tests). |
...
**Group B complete (3/3 surfaces, option c).** All 3 wizard steps migrated to read-only checkpoints; data-integrity bug in WorkflowStepDowntime fixed; mock-fixture fallbacks removed across all 3 steps; useNotificationStore wired for fetch errors. **13 surfaces remaining out of 20 total. Group A + B = 7/20 complete.**
...
**Group E complete (2/2 surfaces).** Defect Types + Part Opportunities migrated to inline-edit AG Grid pattern. Pattern proven for admin catalog surfaces: existing rows autosave PUT on cell change; new rows accumulate locally with explicit Save button.

**Group F complete (1/1 surface).** BOM master-detail rebuilt as two stacked AGGridBase grids — Community-Edition equivalent of master-detail. Picks up the previously-orphaned component-level endpoints from Phase 0 § 6.7.

**Group G complete (2/2 surfaces).** WorkflowStepTargets cleaned up to match the live shape (a review checkpoint, not the 24-hour target grid the migration plan imagined); KPITrackingPanel rebuilt on AGGridBase with variance-classification chip renderer. **18/20 surfaces complete (90%).**

**Group D complete (5/5 surfaces).** All capacity worksheets except BOM (deferred to Group F for master-detail) rebuilt on AGGridBase. Pattern proven: store-bound composable + native AG Grid editors per backend Pydantic schema + workbook-level save unchanged.
...
| Standards (row 21) | ✅ Migrated | `useStandardsGridData.ts` |
| Calendar (row 22) | ✅ Migrated | `useCalendarGridData.ts` |
| Orders (row 20) | ✅ Migrated | `useOrdersGridData.ts` |
| BOM (row 23) | Deferred → Group F (master-detail) | — |

**Group H surface #19 (Work Order Management, row 11) — ✅ Migrated.** v-data-table list + 12-field create/edit dialog replaced with `AGGridBase` + `useWorkOrderGridData`. Inline-edit autosave for existing rows; explicit save for new rows. Status/priority backend-enum-aligned; client_id resolved from auth/kpi store (legacy hardcoded `'CLIENT001'` removed). Filters, summary cards, detail drawer, delete confirmation, more-actions status menu preserved. Tests: `useWorkOrderGridData.spec.ts` (30 tests, all passing).

**Group H surface #20 (Scenarios panel, row 30) — ✅ Migrated.** Card grid + 16-input dialog replaced with `AGGridBase` + `useScenariosGridData`. New rows only (backend has no scenario update endpoint); type-switch resets parameters to type defaults. Run / Delete row actions; multi-row selection drives the Compare button. Bug-fix included: `capacityPlanning.ts:createScenario` payload field renamed `name` → `scenario_name` to match backend ScenarioCreate (legacy 422 was swallowed silently). Tests: `useScenariosGridData.spec.ts` (33 tests, all passing).

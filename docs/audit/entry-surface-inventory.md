...
| 5 | Attendance Entry (form) | `frontend/src/components/entries/AttendanceEntry.vue` | `/data-entry/attendance` | Daily attendance status per employee | `POST /attendance`, `PUT /attendance/{id}`, `POST /attendance/bulk`, `POST /attendance/mark-all-present` | Form | **Non-compliant** | `v-form` at line 16; 6+ `v-text-field`/`v-select` inputs; single-row entry. **Missing:** all Spreadsheet items. A grid implementation exists at `frontend/src/components/grids/AttendanceEntryGrid.vue` but is not the routed target. |
| 6 | Attendance Entry (grid) | `frontend/src/components/grids/AttendanceEntryGrid.vue` | (not routed) | Attendance | `POST /attendance` | Grid | **UI-compliant; payload-divergent** (per §1.2) | Wraps `AGGridBase` (line 112). **Save payload diverges from backend** — see `group-a-runtime-validation.md` §2. Renames (`date`→`shift_date`, `clock_in/out`→`arrival/departure_time`), status enum translation, missing required fields (`client_id`, `scheduled_hours`), and clipboardParser stale entry need fixing. |
| 7 | Downtime Entry (form) | `frontend/src/components/entries/DowntimeEntry.vue` | `/data-entry/downtime` | Downtime events (equipment, reason, start/end time) | `POST /downtime`, `PUT /downtime/{id}` | Form | **Non-compliant** | `v-form` line 16; `v-select`+`v-text-field` for equipment, reason, datetime-local start/end. **Missing:** all Spreadsheet items. Grid exists at `frontend/src/components/grids/DowntimeEntryGrid.vue` but is not routed. |
| 8 | Downtime Entry (grid) | `frontend/src/components/grids/DowntimeEntryGrid.vue` | (not routed) | Downtime events | `POST /downtime`, `PUT /downtime/{id}` | Grid | **UI-compliant; payload-divergent** (per §1.2) | Verified in `useDowntimeGridData.ts:199-294`: typed editors confirmed. **Save payload most divergent of the 4** — see `group-a-runtime-validation.md` §3. Needs enum-mapping table (UI category → `DowntimeReasonEnum`), unit conversion (hours → minutes), missing required fields (`client_id`, `shift_date`), clipboardParser stale, several dead UI fields. |
| 9 | Hold/Resume Entry (form) | `frontend/src/components/entries/HoldResumeEntry.vue` | `/data-entry/hold-resume` | WIP holds and resumes | `POST /holds`, `PUT /holds/{id}`, `POST /holds/{id}/approve-hold`, `POST /holds/{id}/request-resume`, `POST /holds/{id}/approve-resume` | Form (tabbed) | **Non-compliant** | Tabbed `v-form`s (lines 24+) for create-hold and resume; 8+ `v-text-field`/`v-select` inputs. **Missing:** all Spreadsheet items. A grid exists at `frontend/src/components/grids/HoldEntryGrid.vue` but is not routed. |
| 10 | Hold/Resume Entry (grid) | `frontend/src/components/grids/HoldEntryGrid.vue` | (not routed) | WIP holds | `POST /holds`, `PUT /holds/{id}`, `POST /holds/{id}/approve-hold`, `POST /holds/{id}/request-resume`, `POST /holds/{id}/approve-resume` | Grid | **UI-compliant; payload-divergent** (per §1.2) | `AGGridBase` (line 82); filter row uses `v-text-field`/`v-select` which is acceptable per Exception 3. **Save payload diverges** — see `group-a-runtime-validation.md` §4. Renames (`placed_on_hold_date`→`hold_date`, `expected_resume_date`→`expected_resolution_date`), reason-code mapping table, missing `client_id`, server-controlled fields to drop. Approval-workflow buttons already correct. |
...
| 43 | Workflow Wizard — Production step | `frontend/src/components/workflow/steps/WorkflowStepProduction.vue` | Inside guided shift wizard (triggered from MyShift) | Hourly production data point | `POST /production` (via `workflowStore`) | Form | **Non-compliant** | `v-text-field` at lines 62, 76. Single-row guided entry. **Missing:** all Spreadsheet items. Could be a grid for the whole shift. |
...
| 48 | Workflow Wizard — Downtime step | `frontend/src/components/workflow/steps/WorkflowStepDowntime.vue` | shift wizard | Downtime events during shift | `POST /downtime` | Form | **Non-compliant** | `v-text-field`/`v-select` at lines 179, 188. Single-row entry per downtime event. **Missing:** all Spreadsheet items. Should reuse the Downtime grid. |
...
| A | Holds re-route (rows 9 → 10) | Pending |
| A | Attendance re-route (rows 5 → 6) | Pending |
| A | Downtime re-route (rows 7 → 8) | Pending |

**Migration target count after Group A Quality:** 19 surfaces remaining.

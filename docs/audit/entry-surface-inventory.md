...
**Group E in progress (1/2 surfaces).** Defect Types catalog migrated to inline-edit AG Grid pattern (different from Group D's workbook-style — admin catalogs autosave per cell change for existing rows, explicit Save button for new rows).

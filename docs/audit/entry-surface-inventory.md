...
**Group G in progress (1/2 surfaces).** WorkflowStepTargets cleaned up to match the live shape (a review checkpoint, not the 24-hour target grid the migration plan imagined). Same Group B option (c) pattern: canonical `/work-orders` read, no mock fallback, useNotificationStore wired. **17/20 surfaces complete (85%).**

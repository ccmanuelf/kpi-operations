...
### 3. Remove dead code + fix the tests that lock in the bug
- Delete the unused, wrong-path helpers in `services/api/reports.ts`: `getDailyReport`, `getWeeklyReport`, `getMonthlyReport`, and the `exportPDF`/`exportExcel` (bare `/reports/pdf|excel`) helpers — confirmed **no callers** anywhere in `src/` (only their own tests). Keep the working helpers: `getEmailReportConfig`, `saveEmailReportConfig`, `updateEmailReportConfig`, `sendTestEmail`, `triggerManualReport` (correct `/send-manual` path).
- Rewrite `reports.spec.ts`: remove the describe-blocks for the deleted helpers (they assert 404 paths); keep/adjust the email-config + `triggerManualReport` tests (correct contracts).

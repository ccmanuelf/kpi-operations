...
class DateRangeConfig(BaseModel):
    ...
    relative_days: Optional[int] = Field(
        default=7, ge=1, le=365, description="Number of days for relative range (1-365)"
    )
    start_date: Optional[date] = Field(default=None, description="Start date for absolute range")
    end_date: Optional[date] = Field(default=None, description="End date for absolute range")
...
class KPIThresholds(BaseModel):
    ...
    efficiency_min: Optional[float] = Field(default=None, ge=0, le=200, description="Minimum efficiency percentage")
    efficiency_max: Optional[float] = Field(default=None, ge=0, le=200, description="Maximum efficiency percentage")
    quality_rate_min: Optional[float] = Field(default=None, ge=0, le=100, description="Minimum quality rate percentage")
    defect_rate_max: Optional[float] = Field(default=None, ge=0, le=100, description="Maximum defect rate percentage")
    absenteeism_max: Optional[float] = Field(
        default=None, ge=0, le=100, description="Maximum absenteeism rate percentage"
    )
    otd_min: Optional[float] = Field(default=None, ge=0, le=100, description="Minimum on-time delivery percentage")
...
class FilterConfig(BaseModel):
    ...
    client_id: Optional[str] = Field(default=None, max_length=50, description="Filter by specific client ID")
    date_range: Optional[DateRangeConfig] = Field(
        default_factory=lambda: DateRangeConfig(), description="Date range configuration"
    )
    shift_ids: Optional[List[int]] = Field(default=None, description="List of shift IDs to include")
    product_ids: Optional[List[int]] = Field(default=None, description="List of product IDs to include")
    work_order_ids: Optional[List[str]] = Field(default=None, description="List of work order IDs to include")
    employee_ids: Optional[List[int]] = Field(default=None, description="List of employee IDs to include")
    status_filter: Optional[List[str]] = Field(default=None, description="List of statuses to include")
    kpi_thresholds: Optional[KPIThresholds] = Field(default=None, description="KPI threshold filters")
    custom_fields: Optional[Dict[str, Any]] = Field(default=None, description="Custom filter fields for extensibility")
...
class SavedFilterCreate(BaseModel):
    ...
    filter_name: str = Field(..., min_length=1, max_length=100, description="Human-readable filter name")
    filter_type: FilterType = Field(..., description="Filter category/type")
...
class SavedFilterUpdate(BaseModel):
    ...
    filter_name: Optional[str] = Field(
        default=None, min_length=1, max_length=100, description="Human-readable filter name"
    )
    filter_type: Optional[FilterType] = Field(default=None, description="Filter category/type")
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "filter_name": "Weekly Production Overview",
                "filter_type": "production",
                "filter_config": {"client_id": "BOOT-LINE-A", "date_range": {"type": "relative", "relative_days": 7}},
                "is_default": True,
                "usage_count": 15,
...
class FilterHistoryCreate(BaseModel):
    ...
    filter_config: FilterConfig = Field(..., description="Filter configuration that was applied")

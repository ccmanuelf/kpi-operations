...
def _auto_seed_demo_data() -> None:
    ...
    FORCE_RESEED=true: always re-seed (reset=True) (one-time migration).
    Otherwise: smart detection — re-seed only if data is missing/stale.

    The DEMO_MODE gate must stay the first statement so a non-demo deployment
    can never reach the seeder (Run 7 C-1). Guarded by
    backend/tests/test_demo_seed_gate.py.

    Seeding runs through backend.seed, which is INSERT-only and scoped to its
    own allowlist of demo tenants — Alembic is the single schema mechanism
    (C5), so nothing on this path creates or drops a table, and there is no
    longer a destructive branch to guard or to make fatal on failure. The
    whole step is best-effort: a failed seed is logged and swallowed, and the
    app keeps serving (without, or with partial, demo data) rather than
    crashing the boot.
...
def _check_and_seed_demo_data() -> None:
    ...
    try:
        ...
        expected_clients = _expected_clients()
        ...
        if force_reseed:
            logger.info(
                "FORCE_RESEED enabled — re-seeding (%d clients existed)",
                client_count,
            )
            need_seed = True
        elif client_count == 0:
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # Demo seeding is entirely best-effort: backend.seed is INSERT-only and
    # scoped to its own allowlist (Alembic owns schema, so nothing here can
    # half-drop it), so there is no longer a fatal failure mode to re-raise.

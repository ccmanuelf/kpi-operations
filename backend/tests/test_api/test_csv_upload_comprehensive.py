...
class TestCSVUploadEndpoints:
    ...
    def test_attendance_csv_upload(self):
        ...
        writer = csv.DictWriter(
            csv_data,
            fieldnames=["employee_id", "shift_id", "attendance_date", "status", "scheduled_hours", "actual_hours"],
        )
        writer.writeheader()
        writer.writerow(
            {
    ...
    def test_holds_csv_upload(self):
        ...
        csv_data = io.StringIO()
        writer = csv.DictWriter(
            csv_data, fieldnames=["work_order_id", "placed_on_hold_date", "hold_reason", "units_on_hold", "notes"]
        )
        writer.writeheader()
        writer.writerow(
            {
    ...
    def test_csv_large_file_handling(self):
        ...
        for i in range(1000):
            writer.writerow(
                {"work_order_id": f"WO-{i:04d}", "employee_id": f"EMP-{i:04d}", "units_produced": str(100 + i)}
            )
        csv_data.seek(0)
...
class TestCSVUploadResponse:
    ...
    def test_response_structure(self):
        ...
        response = {
            ...
            "successful": 8,
            "failed": 2,
            "errors": [{"row": 3, "error": "Invalid quantity"}, {"row": 7, "error": "Missing required field"}],

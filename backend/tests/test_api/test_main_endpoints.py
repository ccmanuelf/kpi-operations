...
class TestAuthenticationEndpoints:
    ...
    def test_login_nonexistent_user(self, test_client):
        ...
        response = test_client.post(
            "/api/auth/login", json={"username": "nonexistent_user_xyz", "password": "AnyPassword123!"}
        )
    ...
    def test_forgot_password_nonexistent_email(self, test_client):
        ...
        response = test_client.post("/api/auth/forgot-password", json={"email": "nonexistent_xyz@example.com"})
...
class TestAuthenticatedEndpoints:
    ...
    def test_login_success(self, test_client, test_user_data):
        ...
        # Then login
        response = test_client.post(
            "/api/auth/login", json={"username": register_data["username"], "password": register_data["password"]}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_login_invalid_password(self, test_client, test_user_data):
        ...
        response = test_client.post(
            "/api/auth/login", json={"username": register_data["username"], "password": "WrongPassword!"}
        )
        assert response.status_code == 401
    ...
    def test_forgot_password(self, test_client, test_user_data):
        ...
        response = test_client.post("/api/auth/forgot-password", json={"email": test_user_data["email"]})
        assert response.status_code == 200
...
class TestProductionEntryEndpoints:
    ...
    def test_update_production_not_found(self, test_client, admin_auth_headers):
        ...
        response = test_client.put("/api/production/99999", headers=admin_auth_headers, json={"units_produced": 100})
        assert response.status_code == 404
...
class TestKPICalculationEndpoints:
    """Test KPI calculation endpoints - admin sees aggregated cross-tenant data"""

    def test_calculate_kpi_not_found(self, test_client, admin_auth_headers):
        """Test KPI calculation for non-existent entry"""
        response = test_client.get("/api/kpi/calculate/99999", headers=admin_auth_headers)
        assert response.status_code == 404
    ...
    def test_availability_kpi(self, test_client, admin_auth_headers):
        ...
        response = test_client.get(
            f"/api/kpi/availability?product_id=1&shift_id=1&production_date={date.today()}", headers=admin_auth_headers
        )
        assert response.status_code == 200
    ...
    def test_wip_aging_with_product_filter(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/kpi/wip-aging?product_id=1", headers=admin_auth_headers)
        assert response.status_code == 200

    def test_chronic_holds(self, test_client, admin_auth_headers):
        """Test chronic holds endpoint"""
        response = test_client.get("/api/kpi/chronic-holds?threshold_days=30", headers=admin_auth_headers)
        assert response.status_code == 200
    ...
    def test_otd_kpi(self, test_client, admin_auth_headers):
        ...
        response = test_client.get(f"/api/kpi/otd?start_date={start}&end_date={end}", headers=admin_auth_headers)
        assert response.status_code == 200
...
class TestClientEndpoints:
    ...
    def test_get_client_not_found(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/clients/NONEXISTENT-CLIENT-XYZ", headers=admin_auth_headers)
        assert response.status_code == 404

    def test_update_client_not_found(self, test_client, admin_auth_headers):
        """Test updating non-existent client"""
        response = test_client.put(
            "/api/clients/NONEXISTENT-XYZ", headers=admin_auth_headers, json={"client_name": "Test"}
        )
        assert response.status_code == 404
...
class TestEmployeeEndpoints:
    """Test employee management endpoints - admin can list employees"""
    ...
    def test_list_employees_with_pagination(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/employees?skip=0&limit=10", headers=admin_auth_headers)
        assert response.status_code == 200

    def test_list_employees_by_client(self, test_client, admin_auth_headers):
        """Test filtering employees by client"""
        response = test_client.get("/api/employees?client_id=TEST-CLIENT", headers=admin_auth_headers)
        assert response.status_code == 200

    def test_list_floating_pool_employees(self, test_client, admin_auth_headers):
        """Test filtering floating pool employees"""
        response = test_client.get("/api/employees?is_floating_pool=true", headers=admin_auth_headers)
        assert response.status_code == 200
    ...
    def test_get_floating_pool_list(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/employees/floating-pool/list", headers=admin_auth_headers)
        assert response.status_code == 200
...
class TestWorkOrderEndpoints:
    ...
    def test_get_work_order_not_found(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/work-orders/WO-NONEXISTENT-XYZ", headers=admin_auth_headers)
        assert response.status_code == 404

    def test_update_work_order_not_found(self, test_client, admin_auth_headers):
        """Test updating non-existent work order"""
        response = test_client.put(
            "/api/work-orders/WO-NONEXISTENT-XYZ", headers=admin_auth_headers, json={"planned_quantity": 1000}
        )
        assert response.status_code == 404
...
class TestDowntimeEndpoints:
    ...
    def test_update_downtime_not_found(self, test_client, admin_auth_headers):
        ...
        response = test_client.put("/api/downtime/99999", headers=admin_auth_headers, json={"downtime_minutes": 60})
        assert response.status_code == 404
...
class TestHoldEndpoints:
    ...
    def test_update_hold_not_found(self, test_client, admin_auth_headers):
        ...
        response = test_client.put("/api/holds/99999", headers=admin_auth_headers, json={"quantity_held": 100})
        assert response.status_code == 404
...
class TestFloatingPoolEndpoints:
    ...
    def test_list_floating_pool_with_pagination(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/floating-pool?skip=0&limit=10", headers=admin_auth_headers)
        assert response.status_code == 200

    def test_list_floating_pool_available(self, test_client, admin_auth_headers):
        """Test listing available floating pool employees"""
        response = test_client.get("/api/floating-pool?available_only=true", headers=admin_auth_headers)
        assert response.status_code == 200
    ...
    def test_get_available_floating_pool(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/floating-pool/available/list", headers=admin_auth_headers)
        assert response.status_code == 200
...
class TestReportEndpoints:
    ...
    def test_daily_pdf_report(self, test_client, admin_auth_headers):
        ...
        response = test_client.get(f"/api/reports/daily/{date.today()}", headers=admin_auth_headers)
        # Endpoint not implemented in current router
        assert response.status_code == 404
    ...
    def test_pdf_report_with_params(self, test_client, admin_auth_headers):
        ...
        response = test_client.get(f"/api/reports/pdf?start_date={start}&end_date={end}", headers=admin_auth_headers)
        assert response.status_code == 404
...
class TestImportLogEndpoints:
    ...
    def test_get_import_logs_with_limit(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/import-logs?limit=10", headers=admin_auth_headers)
        assert response.status_code == 200
...
class TestErrorHandling:
    ...
    def test_404_for_unknown_endpoint(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/unknown-endpoint-xyz", headers=admin_auth_headers)
        assert response.status_code == 404
    ...
    def test_invalid_date_format(self, test_client, admin_auth_headers):
        ...
        response = test_client.get("/api/production?start_date=invalid-date", headers=admin_auth_headers)
        assert response.status_code == 422

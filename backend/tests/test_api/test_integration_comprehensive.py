...
class TestAttendanceRoutesComprehensive:
    ...
    def test_get_attendance_with_pagination(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/attendance", params={"skip": 0, "limit": 10})
        assert response.status_code in [200, 403]

    def test_get_attendance_by_employee(self, authenticated_client):
        """Test attendance filtered by employee"""
        response = authenticated_client.get("/api/attendance", params={"employee_id": "EMP-001"})
    ...
    def test_get_attendance_by_shift(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/attendance", params={"shift_id": "DAY-1"})
        assert response.status_code in [200, 403, 404, 422]

    def test_get_absenteeism_rate(self, authenticated_client):
        """Test absenteeism rate calculation"""
        response = authenticated_client.get("/api/attendance/absenteeism", params={"client_id": "TEST-CLIENT"})
    ...
    def test_get_attendance_summary(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/attendance/summary", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]
    ...
    def test_update_attendance_record(self, authenticated_client):
        ...
        response = authenticated_client.put("/api/attendance/1", json={"status": "present", "actual_hours": 8.5})
        assert response.status_code in [200, 403, 404, 422]

    def test_attendance_overtime_report(self, authenticated_client):
        """Test overtime report"""
        response = authenticated_client.get("/api/attendance/overtime", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]
...
class TestQualityRoutesComprehensive:
    ...
    def test_get_quality_with_pagination(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/quality", params={"skip": 0, "limit": 10})
        assert response.status_code in [200, 403]

    def test_get_quality_ppm(self, authenticated_client):
        """Test PPM calculation"""
        response = authenticated_client.get("/api/quality/kpi/ppm", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 422]

    def test_get_quality_dpmo(self, authenticated_client):
        """Test DPMO calculation"""
        response = authenticated_client.get("/api/quality/kpi/dpmo", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 422]

    def test_get_quality_fpy(self, authenticated_client):
        """Test FPY calculation"""
        response = authenticated_client.get("/api/quality/kpi/fpy", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_get_quality_rty(self, authenticated_client):
        """Test RTY calculation"""
        response = authenticated_client.get("/api/quality/kpi/rty", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_get_quality_summary(self, authenticated_client):
        """Test quality summary"""
        response = authenticated_client.get("/api/quality/summary", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_get_quality_by_product(self, authenticated_client):
        """Test quality filtered by product"""
        response = authenticated_client.get("/api/quality", params={"product_id": "PROD-001"})
        assert response.status_code in [200, 403, 404, 422]

    def test_get_quality_by_inspection_stage(self, authenticated_client):
        """Test quality filtered by inspection stage"""
        response = authenticated_client.get("/api/quality", params={"inspection_stage": "incoming"})
        assert response.status_code in [200, 403, 404, 422]
    ...
    def test_get_defect_analysis(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/quality/defects/analysis", params={"client_id": "TEST-CLIENT"})
...
class TestCoverageRoutesComprehensive:
    ...
    def test_get_coverage_by_shift(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/coverage", params={"shift_id": "DAY-1"})
        assert response.status_code in [200, 403, 404, 422]

    def test_get_coverage_summary(self, authenticated_client):
        """Test coverage summary"""
        response = authenticated_client.get("/api/coverage/summary", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]
...
class TestDefectRoutesComprehensive:
    ...
    def test_get_defects_by_type(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/defects", params={"defect_type": "dimensional"})
        assert response.status_code in [200, 403, 404]

    def test_get_defects_by_severity(self, authenticated_client):
        """Test defects filtered by severity"""
        response = authenticated_client.get("/api/defects", params={"severity": "critical"})
        assert response.status_code in [200, 403, 404]

    def test_get_defect_summary(self, authenticated_client):
        """Test defect summary"""
        response = authenticated_client.get("/api/defects/summary", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404]
...
class TestReportsRoutesComprehensive:
    ...
    def test_generate_production_report(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/reports/production", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_generate_quality_report(self, authenticated_client):
        """Test generating quality report"""
        response = authenticated_client.get("/api/reports/quality", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_generate_attendance_report(self, authenticated_client):
        """Test generating attendance report"""
        response = authenticated_client.get("/api/reports/attendance", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_generate_downtime_report(self, authenticated_client):
        """Test generating downtime report"""
        response = authenticated_client.get("/api/reports/downtime", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404, 422]

    def test_export_report_csv(self, authenticated_client):
        """Test exporting report as CSV"""
        response = authenticated_client.get(
            "/api/reports/export/csv", params={"client_id": "TEST-CLIENT", "report_type": "production"}
        )
        assert response.status_code in [200, 403, 404]
    ...
    def test_export_report_pdf(self, authenticated_client):
        ...
        response = authenticated_client.get(
            "/api/reports/export/pdf", params={"client_id": "TEST-CLIENT", "report_type": "production"}
        )
        assert response.status_code in [200, 403, 404]
    ...
    def test_generate_daily_summary_report(self, authenticated_client):
        ...
        response = authenticated_client.get(
            "/api/reports/daily-summary", params={"client_id": "TEST-CLIENT", "report_date": date.today().isoformat()}
        )
        assert response.status_code in [200, 403, 404]

    def test_generate_weekly_summary_report(self, authenticated_client):
        """Test generating weekly summary report"""
        response = authenticated_client.get("/api/reports/weekly-summary", params={"client_id": "TEST-CLIENT"})
        assert response.status_code in [200, 403, 404]

    def test_generate_monthly_kpi_report(self, authenticated_client):
        """Test generating monthly KPI report"""
        response = authenticated_client.get(
            "/api/reports/monthly-kpi", params={"client_id": "TEST-CLIENT", "year": 2024, "month": 12}
        )
        assert response.status_code in [200, 403, 404]
...
class TestMultiTenantIsolation:
    ...
    def test_production_client_isolation(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/production", params={"client_id": "OTHER-CLIENT"})
    ...
    def test_downtime_client_isolation(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/downtime", params={"client_id": "OTHER-CLIENT"})
        assert response.status_code in [200, 403]

    def test_quality_client_isolation(self, authenticated_client):
        """Test quality data is isolated by client"""
        response = authenticated_client.get("/api/quality", params={"client_id": "OTHER-CLIENT"})
        assert response.status_code in [200, 403]

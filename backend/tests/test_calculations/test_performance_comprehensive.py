...
class TestShiftPerformance:
    ...
    def test_calculate_shift_performance(self):
        ...
        shift_data = {"shift_id": 1, "planned_output": 500, "actual_output": 475, "defective": 10}

        performance = (shift_data["actual_output"] / shift_data["planned_output"]) * 100
        quality = ((shift_data["actual_output"] - shift_data["defective"]) / shift_data["actual_output"]) * 100
...
class TestPerformanceIntegration:
    ...
    def test_get_performance_for_work_order(self, mock_db):
        ...
        mock_wo = MagicMock(work_order_id="WO-001", planned_quantity=1000, completed_quantity=950)
...
def perf_setup(perf_db):
    ...
    client = TestDataFactory.create_client(
        db, client_id="PERF-TEST-CLIENT", client_name="Performance Test Client", client_type=ClientType.HOURLY_RATE
    )

    # Create user
    supervisor = TestDataFactory.create_user(
        db, user_id="perf-super-001", username="perf_supervisor", role="supervisor", client_id=client.client_id
    )
    ...
    shift = TestDataFactory.create_shift(
        db, client_id=client.client_id, shift_name="Performance Shift", start_time="06:00:00", end_time="14:00:00"
    )
...
class TestCalculatePerformanceReal:
    ...
    def test_calculate_performance_100_percent(self, perf_setup):
        ...
        performance, ideal_cycle_time, was_inferred = calculate_performance(db, entry, product)
...
class TestPerformanceWithInference:
    ...
    def test_performance_with_inferred_cycle_time(self, perf_setup):
        ...
        performance, ideal_cycle_time, was_inferred = calculate_performance(db, entry, product_no_ict)

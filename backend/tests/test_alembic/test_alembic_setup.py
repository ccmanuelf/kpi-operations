...
class TestModelMetadata:
    ...
    def test_metadata_table_count_minimum(self):
        ...
        total to 60; 0007_transaction_soft_delete.py adds only columns, so the
        count is unchanged).
        """
        from backend.database import Base
...
class TestAlembicCLI:
    ...
    def test_alembic_heads(self):
        ...
        assert (
            "0009_global_kpi_targets" in result.stdout
        ), f"0009_global_kpi_targets not in heads output: {result.stdout}"
    ...
    def test_alembic_current_after_stamp(self, tmp_path):
        ...
        assert (
            "0009_global_kpi_targets" in result.stdout
        ), f"Expected 0009_global_kpi_targets in current output: {result.stdout}"

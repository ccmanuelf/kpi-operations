...
# ---------------------------------------------------------------------------
# NULLS LAST guard (always-on, dialect-agnostic). SQLAlchemy emits the NULLS
...
def test_no_nullslast_in_query_construction():
    ...
    those that do not, so ascending on it reproduces NULLS LAST exactly.
    """
    import pathlib
    import re
    ...
    pattern = re.compile(r"\bnulls(last|first)\s*\(")  # nullslast-guard: allow
...
def test_nulls_last_clause_is_rejected_but_is_null_ordering_works():
    ...
    try:
        ...
        try:
            session.execute(text("SELECT id FROM _nl ORDER BY u DESC NULLS LAST"))
            raise AssertionError("MariaDB accepted NULLS LAST -- the guard's premise no longer holds")

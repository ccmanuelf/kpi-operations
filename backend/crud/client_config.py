...
def create_client_config(
    ...
    existing = db.query(ClientConfig).filter(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail=f"Configuration already exists for client {client_id}. Use update instead."
        )
...
def get_client_config_or_defaults(
    db: Session,
    client_id: str
) -> dict:
    """
...
def update_client_config(
    ...
    if not db_config:
        raise HTTPException(
            status_code=404,
            detail=f"Configuration not found for client {client_id}. Create it first."
        )
...
def delete_client_config(
    ...
    if current_user.role != 'admin':
        raise HTTPException(
            status_code=403,
            detail="Only admins can delete client configurations"
        )
    ...
    if not db_config:
        raise HTTPException(
            status_code=404,
            detail=f"Configuration not found for client {client_id}"
        )
...
def get_all_client_configs(
    ...
    if current_user.role != 'admin':
        raise HTTPException(
            status_code=403,
            detail="Only admins can view all client configurations"
        )

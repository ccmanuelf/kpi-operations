...
def create_client(db: Session, client_data: dict, current_user: User) -> Client:
    """
    Create new client
    ...
    existing = db.query(Client).filter(Client.client_id == client_data.get("client_id")).first()

    if existing:
        raise HTTPException(status_code=400, detail=f"Client with ID {client_data.get('client_id')} already exists")
...
def get_client(db: Session, client_id: str, current_user: User) -> Optional[Client]:
    """
    Get client by ID
...
def get_clients(
    db: Session, current_user: User, skip: int = 0, limit: int = 100, is_active: Optional[bool] = None
) -> List[Client]:
    """
    ...
    return query.order_by(Client.client_name).offset(skip).limit(limit).all()


def update_client(db: Session, client_id: str, client_update: dict, current_user: User) -> Optional[Client]:
    """
    Update client
    SECURITY: Verifies user has access to the client
...
def delete_client(db: Session, client_id: str, current_user: User) -> bool:
    """
...
def get_active_clients(db: Session, current_user: User, skip: int = 0, limit: int = 100) -> List[Client]:
    """

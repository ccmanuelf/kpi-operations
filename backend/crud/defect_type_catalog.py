...
def create_defect_type(
    ...
    existing = db.query(DefectTypeCatalog).filter(
        and_(
...
def get_defect_type(
    ...
    defect_type = db.query(DefectTypeCatalog).filter(
        DefectTypeCatalog.defect_type_id == defect_type_id
    ).first()
...
def get_defect_types_by_client(
    ...
    if is_global_client(client_id):
        ...
        client_filter = or_(
            ...
            DefectTypeCatalog.client_id == GLOBAL_CLIENT_ID
        )
    else:
...
def get_global_defect_types(
    db: Session,
    include_inactive: bool = False
) -> List[DefectTypeCatalog]:
    """Get all global defect types (available to all clients)"""
    query = db.query(DefectTypeCatalog).filter(
        DefectTypeCatalog.client_id == GLOBAL_CLIENT_ID
    )
...
def update_defect_type(
    ...
    db_defect_type = db.query(DefectTypeCatalog).filter(
        DefectTypeCatalog.defect_type_id == defect_type_id
    ).first()
...
def bulk_create_defect_types(
    ...
    if replace_existing:
        ...
        db.query(DefectTypeCatalog).filter(
            DefectTypeCatalog.client_id == client_id
    ...
    for idx, dt in enumerate(defect_types):
        try:
            ...
            existing = db.query(DefectTypeCatalog).filter(
                and_(
                    DefectTypeCatalog.client_id == client_id,
                    DefectTypeCatalog.defect_code == dt.defect_code
                )
            ).first()
    ...
    return {
        ...
        "skipped": skipped,
        "errors": errors,
        "total_processed": len(defect_types)
    }


def validate_defect_type_for_client(
    db: Session,
    client_id: str,
    ...
    """
    exists = db.query(DefectTypeCatalog).filter(
        and_(
            DefectTypeCatalog.client_id == client_id,
...
def get_defect_type_by_name(
    db: Session,
    client_id: str,
    defect_type_name: str
) -> Optional[DefectTypeCatalog]:
    """Get defect type by name for a client"""
    return db.query(DefectTypeCatalog).filter(
        and_(
            DefectTypeCatalog.client_id == client_id,
            DefectTypeCatalog.defect_name == defect_type_name,
            DefectTypeCatalog.is_active == True

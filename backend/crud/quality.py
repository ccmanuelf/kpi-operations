...
def _calculate_quality_kpis(
    units_inspected: int, units_defective: int, opportunities_per_unit: int = 10
...
def create_quality_inspection(
    db: Session, inspection: QualityInspectionCreate, current_user: User
...
def get_quality_inspection(db: Session, inspection_id: str, current_user: User) -> Optional[QualityEntry]:
    ...
    """
    db_inspection = db.query(QualityEntry).filter(QualityEntry.quality_entry_id == inspection_id).first()
...
def get_quality_inspections(
    ...
    return query.order_by(QualityEntry.shift_date.desc()).offset(skip).limit(limit).all()


def update_quality_inspection(
    db: Session, inspection_id: str, inspection_update: QualityInspectionUpdate, current_user: User

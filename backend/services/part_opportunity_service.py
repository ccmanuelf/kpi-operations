...
def create_opportunity(db: Session, part_data: dict, current_user: User):
    """Create a new part opportunity."""
    return create_part_opportunity(db, part_data, current_user)


def get_opportunity(db: Session, part_number: str, current_user: User):
    """Get a part opportunity by part number."""
    return get_part_opportunity(db, part_number, current_user)


def list_opportunities(db: Session, current_user: User, skip: int = 0, limit: int = 100):
    """List part opportunities."""
    return get_part_opportunities(db, current_user, skip, limit)


def list_opportunities_by_category(db: Session, category: str, current_user: User):
    """List part opportunities by category."""
    return get_part_opportunities_by_category(db, category, current_user)


def update_opportunity(db: Session, part_number: str, part_data: dict, current_user: User):

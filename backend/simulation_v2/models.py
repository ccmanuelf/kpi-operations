...
class OperationInput(BaseModel):
    ...
    product: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Product identifier - must match across operations and demands"
    )
    step: int = Field(
        ...,
        gt=0,
        le=999,
        description="Sequence order within product (1-based, sequential, no gaps)"
    )
    operation: str = Field(
        ...,
        min_length=1,
        max_length=200,
        description="Operation description (e.g., 'Join shoulder seams')"
    )
    machine_tool: str = Field(
        ...,
        min_length=1,
        max_length=100,
        description="Required resource identifier (e.g., '4-thread overlock')"
    )
    sam_min: float = Field(
        ...,
        gt=0,
        le=999,
        description="Standard Allowed Minutes per piece"
    )
    ...
    grouping: str = Field(
        ...
        max_length=50,
        description="Additional categorization for reporting"
    )
    operators: int = Field(
    ...
    variability: VariabilityType = Field(
        default=VariabilityType.TRIANGULAR,
        description="Processing time variability mode"
    )
    rework_pct: float = Field(
        default=DEFAULT_REWORK_PCT,
        ge=0,
        le=100,
        description="Percentage of pieces requiring rework at this operation"
    )
    grade_pct: float = Field(
        default=DEFAULT_GRADE_PCT,
        ge=0,
        le=100,
        description="Operator skill level (100 = fully trained)"
    )
    fpd_pct: float = Field(
        default=DEFAULT_FPD_PCT,
        ge=0,
        le=100,
        description="Fatigue and Personal Delay percentage"
    )
...
class ScheduleConfig(BaseModel):
    ...
    shifts_enabled: int = Field(
        ...,
        ge=1,
        le=3,
        description="Number of shifts per day (1-3)"
    )
    shift1_hours: float = Field(
        ...,
        gt=0,
        le=12,
        description="Shift 1 duration in hours"
    )
    shift2_hours: float = Field(
        default=0.0,
        ge=0,
        le=12,
        description="Shift 2 duration in hours"
    )
    shift3_hours: float = Field(
        default=0.0,
        ge=0,
        le=12,
        description="Shift 3 duration in hours"
    )
    work_days: int = Field(
        ...,
        ge=1,
        le=7,
        description="Work days per week"
    )

    # Overtime configuration
    ot_enabled: bool = Field(
        default=False,
        description="Enable overtime calculation"
    )
    weekday_ot_hours: float = Field(
        default=0.0,
        ge=0,
        le=8,
        description="Additional overtime hours per weekday"
    )
    weekend_ot_days: int = Field(
        default=0,
        ge=0,
        le=2,
        description="Number of weekend days with overtime"
    )
    weekend_ot_hours: float = Field(
        default=0.0,
        ge=0,
        le=12,
        description="Hours per weekend overtime day"
    )
    ...
    def validate_total_hours(self):
        ...
        if total > 24:
            raise ValueError(
                f"Total shift hours ({total:.1f}) cannot exceed 24 hours per day"
            )
        return self
    ...
    def validate_overtime_hours(self):
        ...
        if self.ot_enabled:
            ...
            if max_shift + self.weekday_ot_hours > 24:
                raise ValueError(
                    f"Longest shift ({max_shift:.1f}h) + weekday OT "
                    f"({self.weekday_ot_hours:.1f}h) exceeds 24 hours"
                )
        return self
...
class DemandInput(BaseModel):
    ...
    product: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Must exactly match a product from operations list"
    )
    bundle_size: int = Field(
        default=DEFAULT_BUNDLE_SIZE,
        ge=1,
        le=MAX_BUNDLE_SIZE,
        description="Pieces per bundle"
    )
    daily_demand: Optional[float] = Field(
        default=None,
        ge=0,
        description="Target pieces per day (demand-driven mode)"
    )
    weekly_demand: Optional[float] = Field(
        default=None,
        ge=0,
        description="Target pieces per week (demand-driven mode)"
    )
    mix_share_pct: Optional[float] = Field(
        default=None,
        ge=0,
        le=100,
        description="Percentage of total production (mix-driven mode)"
    )
...
class BreakdownInput(BaseModel):
    ...
    machine_tool: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Must match a machine_tool from operations"
    )
    breakdown_pct: float = Field(
        default=0.0,
        ge=0,
        le=100,
        description="Probability of breakdown during each operation (%)"
    )

    class Config:
        json_schema_extra = {
            "example": {
                "machine_tool": "4-thread overlock",
                "breakdown_pct": 2.5
            }
        }
...
class SimulationConfig(BaseModel):
    ...
    operations: List[OperationInput] = Field(
        ...,
        min_length=1,
        description="List of operations defining production processes"
    )
    schedule: ScheduleConfig = Field(
        ...,
        description="Production schedule configuration"
    )
    demands: List[DemandInput] = Field(
        ...,
        min_length=1,
        description="Demand configuration per product"
    )
    breakdowns: Optional[List[BreakdownInput]] = Field(
        default=None,
        description="Optional equipment breakdown configuration"
    )
    mode: DemandMode = Field(
        default=DemandMode.DEMAND_DRIVEN,
        description="Demand specification mode"
    )
    total_demand: Optional[float] = Field(
        default=None,
        ge=0,
        description="Total daily demand (required for mix-driven mode)"
    )
    ...
    def validate_mix_mode_total_demand(self):
        ...
        if self.mode == DemandMode.MIX_DRIVEN and not self.total_demand:
            raise ValueError(
                "total_demand is required when mode is 'mix-driven'"
            )
        return self
...
class ValidationIssue(BaseModel):
    ...
    category: str = Field(
        ...,
        description="Validation category (e.g., 'sequence', 'demand', 'capacity')"
    )
    message: str = Field(
        ...,
    ...
    field: Optional[str] = Field(
        default=None,
        description="Specific field that caused the issue"
    )
    product: Optional[str] = Field(
        default=None,
        description="Product associated with the issue"
    )

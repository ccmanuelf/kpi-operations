...
def _validate_configuration_limits(config: SimulationConfig, products: Set[str], report: ValidationReport) -> None:
...
def _validate_operation_sequences(
    operations_by_product: Dict[str, List[OperationInput]], report: ValidationReport
) -> None:
...
def _validate_product_consistency(
    operations_by_product: Dict[str, List[OperationInput]], demands: List[DemandInput], report: ValidationReport
) -> None:
...
def _validate_machine_tool_consistency(operations: List[OperationInput], report: ValidationReport) -> None:
    ...
    for i, tool1 in enumerate(tools):
        for tool2 in tools[i + 1 :]:
            ...
            # Check similarity
            ratio = difflib.SequenceMatcher(None, tool1.lower().strip(), tool2.lower().strip()).ratio()
...
def _validate_demand_configuration(config: SimulationConfig, report: ValidationReport) -> None:
...
def _validate_mix_driven_demand(config: SimulationConfig, report: ValidationReport) -> None:
...
def _validate_demand_driven_demand(config: SimulationConfig, report: ValidationReport) -> None:
...
def _validate_breakdown_configuration(
    config: SimulationConfig, machine_tools: Set[str], report: ValidationReport
) -> None:
        ...
        if breakdown.machine_tool not in machine_tools:
            report.warnings.append(
                ValidationIssue(
                    ...
                    field="machine_tool",
                    message=(
                        f"Breakdown configured for '{breakdown.machine_tool}' " f"which is not used in any operation"
                    ),
...
def _validate_theoretical_capacity(
    config: SimulationConfig, operations_by_product: Dict[str, List[OperationInput]], report: ValidationReport
) -> None:

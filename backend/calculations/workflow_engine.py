"""
Workflow Engine - Domain Logic for Work Order State Machine
Implements Phase 10: Flexible Workflow Foundation

This module contains the core workflow domain logic that is shared between
the CRUD layer and the Service layer. It was extracted from
backend/services/workflow_service.py to eliminate the upward CRUD->Service
dependency and establish the proper layering:

    Route -> Service -> CRUD -> ORM
                  \\       /
                   Domain
                (calculations/)

Provides:
- WorkflowStateMachine: State machine validation and configuration
- execute_transition: Execute a validated status transition
- get_workflow_config: Get workflow configuration for a client
- get_transition_history: Get transition log for a work order
- bulk_transition: Bulk status transitions
- apply_workflow_template: Apply workflow template to client

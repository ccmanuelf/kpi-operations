  ...
  <v-dialog v-model="dialog" max-width="1000px" persistent scrollable>
    ...
    <v-card>
      <v-card-title class="bg-primary text-white d-flex justify-space-between align-center">
        <span class="text-h5">CSV Import - Attendance Records</span>
            ...
            <v-card flat>
              <v-card-text>
                <v-alert type="info" variant="tonal" class="mb-4">
                  ...
                  Select a CSV file with attendance records. We'll validate the data before importing.
                </v-alert>
...
const confirmImport = async () => {
  ...
  try {
    ...
    formData.append('file', blob, 'attendance_import.csv')
...
CLIENT001,103,2026-01-20,8.0,1,0.0,8.0,UNSCHEDULED_ABSENCE,0,0,1,Absent - no call no show`
  ...
  link.href = url
  link.setAttribute('download', 'attendance_entry_template.csv')

/**
 * Composable for the Availability KPI chart configuration. Reactive
 * Chart.js datasets and options keyed off `kpi.trends.availability`.
 */
...
export default function useAvailabilityCharts() {
  const { t } = useI18n()
  const kpiStore = useKPIStore()
  const { scaleDefaults, legendDefaults, chartColors } = useChartTheme()

  const chartData = computed(() => {
    const trend = kpiStore.trends.availability as TrendPoint[]
    return {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: Array(trend.length).fill(90),

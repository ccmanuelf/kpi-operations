/**
 * Composable for the OEE KPI chart configuration. Reactive
 * Chart.js datasets and options keyed off `kpi.trends.oee`.
 * Uses the blue palette and a "world-class" 85% target line.
 */
...
export default function useOEECharts() {
  const { t } = useI18n()
  const kpiStore = useKPIStore()
  const { scaleDefaults, legendDefaults, chartColors } = useChartTheme()

  const chartData = computed(() => {
    const trend = kpiStore.trends.oee as TrendPoint[]
    return {

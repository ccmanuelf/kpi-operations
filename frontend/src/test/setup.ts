/**
 * Vitest test setup — global mocks and Vuetify component stubs.
 */
import { vi } from 'vitest'
import { config } from '@vue/test-utils'

// Mock window.confirm
;(globalThis as { confirm: typeof confirm }).confirm = vi.fn(() => true)

// Mock window.URL.createObjectURL / revokeObjectURL
;(globalThis as unknown as { URL: { createObjectURL: () => string; revokeObjectURL: () => void } }).URL.createObjectURL = vi.fn(
  () => 'mocked-url',
)
;(globalThis as unknown as { URL: { revokeObjectURL: () => void } }).URL.revokeObjectURL =
  vi.fn()

// Mock CSS imports
vi.mock('vuetify/styles', () => ({}))

// Vuetify component stubs — minimal templates so component tests can
// render without loading Vuetify itself.
config.global.stubs = {

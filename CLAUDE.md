...
- **Cross-model PR gate.** A project PreToolUse hook (`scripts/cross-review-gate.sh`, wired in `.claude/settings.json`) blocks the agent's `gh pr create`/`gh pr merge` calls unless a cross-model review ran for the current HEAD. Run `/cross-review` to get an adversarial DeepSeek-Reasoner pass on the PR diff (it writes `.git/cross-review/<sha>.json`, local-only); a new commit invalidates it. `/cross-review --skip "<reason>"` bypasses with the reason logged (e.g. MCP unreachable). It is **advisory** — it enforces that a review ran for this commit, not that it passed — and **complements, never replaces, `/code-review`** (a Claude pass). The matcher is conservative: it triggers on `gh pr create/merge` at a command position (line start or after `;`/`&`/`|`), so a separator immediately followed by that phrase even inside a quoted string can trip a spurious block — resolve by running `/cross-review` or restructuring the command. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-24-cross-model-pr-review-gate*`.
...
### Frontend modernization — Vuetify 4 + MD3 migration (SHIPPED 2026-06-15)
Vuetify 3.5→4.1 + Material Design 3 with a Carbon-seeded tonal theme, shipped as 7 PRs (#73–#79, all merged, main CI green; local==GitHub==Render verified). Specs/plans in `docs/superpowers/{specs,plans}/2026-06-14-vuetify4-md3-*`.
- **PR1 (#73) foundation** — Vuetify 4.1.1 + `md3` blueprint + Carbon-seeded MD3 tonal theme (`src/theme/md3Tonal.ts` + `carbonSeeds.ts`). Cascade-layer **keystone fix**: a master `@layer` order in `src/assets/main.css` (imported first) so Tailwind 4's preflight (`base`) sits BELOW Vuetify's component layers — was clipping content under the nav drawer. Tailwind `@theme inline` → Carbon `--cds-*` tokens (killed a stray indigo). WCAG-AA sweep to 0 real failures, light + dark.
- **a11y gate (#74)** — deterministic vitest contrast gate (`src/theme/__tests__/contrast.a11y.spec.ts`) parses `carbon-tokens.css` both themes + asserts AA; runs in `frontend-lint-and-tests` so AA can't silently regress.
- **PR2 (#75) spreadsheet-natural grids** — 38px rows / 40px header (central `useResponsive`/`useAGGridBase`), vertical gridlines, compact square tags (`gridTagStyle.ts`).

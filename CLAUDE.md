...
- **Verify with the repo's existing workflow.** Backend: `pytest` (coverage gate ≥75 %, current 78.66 %). Frontend: `npm run test`, `npm run lint`, coverage thresholds 32/25/25/34 (statements/branches/functions/lines, raised after Phase B.3). Smoke: `/health/live` on the API. Don't invent new validation paths.

...
try:
    ...
    for wo_id, client_id in work_orders:
        ...
        if random.random() < 0.3:
            ...
            released_date = (
                (hold_date + timedelta(days=random.randint(1, 14))).strftime("%Y-%m-%d") if released else None
            )
        ...
        for part_num, part_desc, part_cat, opps in parts:
            ...
            try:
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """,
                    (full_part_num, client_id, opps, part_desc, part_cat, "Auto-generated part opportunity"),
                )
                part_count += 1
            except sqlite3.IntegrityError:
                pass
